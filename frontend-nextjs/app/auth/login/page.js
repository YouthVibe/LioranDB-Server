...
import { SignInButton } from "../../utils/AuthButtons";
...
export default function Page() {
  ...
  // ✅ Handle redirect for logged-in users only once
  useEffect(() => {
    let redirectTimeout;
    ...
    <div className="relative min-h-screen flex items-center justify-center bg-black p-4">
      ...
      <SignedOut>
        <motion.div
          ...
          className={`w-full max-w-sm ${disableCard ? "pointer-events-none" : "pointer-events-auto"
            }`}
        >
          <Card className="backdrop-blur-xl bg-neutral-900/60 border-white/10 text-white shadow-2xl rounded-2xl">
            <CardHeader className="text-center">
              ...
              <p className="text-sm text-gray-400 mt-1">
                Login to continue using Hushar Spreadsheet
              </p>
            </CardHeader>

            <CardContent className="flex flex-col gap-4">
              {/* ✅ Replaced with reusable SignInButton */}
              <SignInButton redirectTo="/spreadsheet">
                <Button
                  variant="outline"
      ...
      <SignedIn>
        ...
        {/* ✅ Redirect only once using useEffect */}
        <RedirectToApp />
      </SignedIn>
    </div>
  );
...
// ✅ Small helper to handle redirect safely (prevents infinite loops)
